...
(function () {
    ...
    init: function (editor) {
      ...
      editor.on(
        ...
        function (evt) {
          ...
          if (value) {
            ...
            if (value) {
              evt.data.dataValue = editor
                .editable()
                .transformPlainTextToHtml(value);
              evt.data.type = "text";
            }
          }
        },
      ...
      editor.on(
        ...
        function (evt) {
          ...
          if (type == "auto") {
            type =
              trueType == "html" || defaultType == "html" ? "html" : "text";
          }
  ...
  function recogniseContentType(data) {
    if (CKEDITOR.env.webkit) {
      ...
      if (
        !data.match(/^[^<]*$/g) &&
        !data.match(/^(<div><br( ?\/)?><\/div>|<div>[^<]*<\/div>)*$/gi)
      )
        return "html";
      ...
      // Text or <br>.
      if (!data.match(/^([^<]|<br( ?\/)?>)*$/gi)) return "html";
    } else {
      return "html";
    }
  ...
  function htmlifiedTextHtmlification(config, data) {
    function repeatParagraphs(repeats) {
      ...
      return (
        CKEDITOR.tools.repeat("</p><p>", ~~(repeats / 2)) +
        (repeats % 2 == 1 ? "<br>" : "")
      );
    }
    ...
    if (CKEDITOR.env.webkit && data.indexOf("<div>") > -1) {
      ...
      if (data.match(/<div>(<br>|)<\/div>/)) {
        data =
          "<p>" +
          data.replace(/(<div>(<br>|)<\/div>)+/g, function (match) {
    ...
    if (CKEDITOR.env.gecko && config.enterMode != CKEDITOR.ENTER_BR) {
      ...
      if (CKEDITOR.env.gecko) data = data.replace(/^<br><br>$/, "<br>");
      ...
      if (data.indexOf("<br><br>") > -1) {
        ...
        data =
          "<p>" +
          data.replace(/(<br>){2,}/g, function (match) {
  ...
  function filtersFactoryFactory() {
    ...
    function isPastedFromGDocs(element) {
      if (
        ...
        element.attributes.id.match(/^docs\-internal\-guid\-/)
      ) {
        return true;
    ...
    function replaceChilds(element) {
      if (!element.children) {
        return;
      }
      ...
      for (let i = 0; i < element.children.length; i++) {
        ...
        if (element.name === 'span'
          ...
          if (styles["font-weight"] > 400) {
            var regex = /font-weight:(.*?);/;
            element.wrapWith(new CKEDITOR.htmlParser.element("b"));
            element.attributes.style = element.attributes.style.replace(regex, '');
          }
        }
      }
    }
    ...
    function processDataFromGDocs(element) {
      var styles =
        element.attributes.style &&
        CKEDITOR.tools.parseCssText(element.attributes.style);

      if (
        element.attributes.id &&
        element.attributes.id.match(/^docs\-internal\-guid\-/)
      ) {
        replaceChilds(element);

        if (element.name === "b") {
          if (
            element.attributes &&
            element.attributes.style &&
            element.attributes.style.indexOf("font-weight:normal;") > -1
          ) {
    ...
    return {
      get: function (type) {
        if (type == "plain-text") {
          ...
          return (
            filters.plainText || (filters.plainText = new CKEDITOR.filter("br"))
          );
        } else if (type == "semantic-content") {
          return (
            filters.semanticContent ||
            (filters.semanticContent = createSemanticContentFilter())
          );
          ...
          return (
            filters.webkitDefaultFilter ||
            (filters.webkitDefaultFilter = createWebkitDefaultFilter())
          );
        } else if (type) {
