...
(function () {
    ...
    CKEDITOR.dialog.add("base64imageDialog", function (editor) {
        ...
        /* Load preview image */
        function imagePreviewLoad(url, fileAsBase64) {
            ...
            /* Load image */
            if (urlBase64Config.converToBase64 && !fileAsBase64) {
                if (!/^https?:\/\//i.test(url))
                    url = "https://" + url;
        ...
        function toDataUrl(url, callback) {
            ...
            xhr.onerror = function () {
                imgPreview.getElement().setHtml(editor.lang.base64image.imageNotFound);
            }
        ...
        function imagePreview(src) {
            ...
            if (src === "base64") {
                ...
                /* Disable Checkboxes */
                if (urlBase64Config.enabled && urlCB)
                ...
                if (urlBase64Config.enabled && urlCB)
                    urlCB.setValue(true, true);
                if (fileCB) fileCB.setValue(false, true);

                /* Load preview image */
                if (urlI) imagePreviewLoad(urlI.getValue(), false);
                ...
                if (n && "files" in n && n.files && n.files.length > 0 && n.files[0]) {
                    ...
                    fr.onload = (function (f) {
                        return function (e) {
                            imgPreview.getElement().setHtml("");
                            imagePreviewLoad(e.target.result, true);
                        };
                    })(n.files[0]);
        ...
        if (fsupport) {
            ...
            if (urlBase64Config.enabled) {
                var urlSourceElement = {
                    type: "hbox",
                    widths: ["70px"],
                    children: [
                        {
                ...
                sourceElements.push(urlSourceElement);
            }
            sourceElements.push({
                    type: "hbox",
                    widths: ["70px"],
        ...
        return {
                ...
                if (selectedImg) {
                    ...
                    if (typeof (selectedImg.getAttribute("src")) == "string") {
                        if (selectedImg.getAttribute("src").indexOf("data:") === 0) {
                            imagePreview("base64");
                            imagePreviewLoad(selectedImg.getAttribute("src"), true);
                        } else {
            ...
            onOk: function () {
                ...
                }, css = [], value, cssvalue, attrvalue, k, unit;
                for (k in attr) {
