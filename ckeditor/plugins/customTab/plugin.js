...
(function () {
                ...
                if (editor.editable().hasFocus) {
                    ...
                    if ((cell = path.contains({ td: 1, th: 1 }, 1))) {
                        ...
                        if (!(next || backward)) {
                            ...
                            var newCells = newRow.getChildren();
                            var selectElement = newCells && newCells.count() > 0 ?
                                newCells.getItem(0) :
                                newRow;

                            if (editor.config.reformatEmptyTableCells) {
                                editor.config.reformatEmptyTableCells(editor, newRow);
                                var pChildren = selectElement.find("p");

                                if (pChildren && pChildren.count() > 0) {
                                    selectElement = pChildren.getItem(0);
                                    var spanChildren = selectElement.find("span");
                                    if (spanChildren && spanChildren.count() > 1) {
                                        selectElement = spanChildren.getItem(1);
                                    }
                                }
                            }

                            resultRange.moveToElementEditStart(selectElement);
                        } else if (next) {
    ...
    CKEDITOR.plugins.add('customTab', {
        init: function (editor) {
            ...
            editor.on(
                ...
                function (ev) {
                    ...
                    var isInTable = selection != null && typeof selection.isInTable !== "undefined" && ev.editor.getSelection().isInTable(true);
                    ...
                    if (
                        ...
                        tabText && ev.data.keyCode == 9 // TAB
                    ) {
                        editor.insertText(tabText)
                        ev.cancel();
                    }
                    else if (ev.data.keyCode == (CKEDITOR.SHIFT + 9)) { // SHIFT+TAB
                        // do nothing if cursor is not inside the table and SHIFT+TAB is pressed
                        // so editor doesn't lose focus
                        // this is added because in fullscreen mode, users lost focus and navigated back using backpace in IE,
                        //or by pressing enter on some focused link/button
                        ev.cancel();
                    }
                }
            );
